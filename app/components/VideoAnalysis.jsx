...
const categoryAccent = {
  'Shooting':      { text: 'text-red-400',    plusBtn: 'bg-red-600 hover:bg-red-500 active:scale-95',       minusBtn: 'bg-gray-700 hover:bg-gray-600 active:scale-95 text-red-400 border border-red-900' },
  'Passing':       { text: 'text-blue-400',   plusBtn: 'bg-blue-600 hover:bg-blue-500 active:scale-95',     minusBtn: 'bg-gray-700 hover:bg-gray-600 active:scale-95 text-blue-400 border border-blue-900' },
  'Dribbling':     { text: 'text-purple-400', plusBtn: 'bg-purple-600 hover:bg-purple-500 active:scale-95', minusBtn: 'bg-gray-700 hover:bg-gray-600 active:scale-95 text-purple-400 border border-purple-900' },
  'Defending':     { text: 'text-teal-400',   plusBtn: 'bg-teal-600 hover:bg-teal-500 active:scale-95',     minusBtn: 'bg-gray-700 hover:bg-gray-600 active:scale-95 text-teal-400 border border-teal-900' },
  'Miscellaneous': { text: 'text-amber-400',  plusBtn: 'bg-amber-500 hover:bg-amber-400 active:scale-95',   minusBtn: 'bg-gray-700 hover:bg-gray-600 active:scale-95 text-amber-400 border border-amber-900' },
  'Goalkeeping':   { text: 'text-green-400',  plusBtn: 'bg-green-600 hover:bg-green-500 active:scale-95',   minusBtn: 'bg-gray-700 hover:bg-gray-600 active:scale-95 text-green-400 border border-green-900' },
};
  ...
  return (
    <div className={`flex flex-col rounded-2xl border-2 ${categoryBorder[group.category]} overflow-hidden w-full`}>
      <div className={`bg-gradient-to-r ${categoryColors[group.category]} px-4 py-3 flex-shrink-0`}>
        <span className="text-white text-xl font-extrabold uppercase tracking-widest">{group.category}</span>
      </div>
      <div className="flex flex-col gap-2 p-2 bg-gray-900">
        {group.stats.map(stat => (
          <StatRow key={stat.id} stat={stat} value={getValue(stat.id)} onBump={bump} accent={accent} />
        ))}
      </div>
    </div>
  );
...
export default function VideoAnalysis({ position, onStatsExtracted, onSkip }) {
  ...
  const groupedStats = (() => {
    ...
    if (isDefensivePosition) {
      return outfieldGroupedStats.map(group =>
        group.category === 'Defending'
          ? { ...group, stats: [...defensive_position_stats, ...group.stats] }
          : group
      );
    }
  ...
  const navProgress = (categoryIndex / totalCategories) * 100;
  ...
  const startProgress = () => {
    ...
    setProgress(0);
    setStageLabel(ANALYSIS_STAGES[0].label);
    const advance = () => {
      const idx = stageIndexRef.current;
      if (idx >= ANALYSIS_STAGES.length - 1) return;
      const next = idx + 1;
      stageIndexRef.current = next;
      setProgress(ANALYSIS_STAGES[next].progress);
      setStageLabel(ANALYSIS_STAGES[next].label);
      progressTimerRef.current = setTimeout(advance, 3000 + Math.random() * 4000);
    };
    progressTimerRef.current = setTimeout(advance, 2000);
  };
      ...
      {(step === 'upload' || step === 'error') && (
        <div className="max-w-2xl mx-auto w-full mb-8">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={onSkip} className="text-gray-500 hover:text-gray-300 text-sm uppercase tracking-widest border border-gray-700 hover:border-gray-500 px-3 py-1 rounded-lg transition-colors">
              Skip → Manual Entry
            </button>
          </div>
          <p className="text-gray-500 text-sm">
            Position: <span className="text-teal-400 font-bold">{positionName}</span>
        ...
        {step === 'analyzing' && (
          <motion.div key="analyzing" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="text-center w-full">
              <p className="text-white text-xl font-bold mb-1">Analyzing your footage...</p>
        ...
        {step === 'error' && (
          <motion.div key="error" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="text-center">
              <p className="text-white text-xl font-bold mb-2">Analysis Failed</p>
          ...
          <motion.div key="review" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex-1 flex flex-col w-full">
            <div className="flex items-center gap-4 mb-6">
              <div className="flex-1 h-2 bg-gray-800 rounded-full overflow-hidden shadow-inner">
                <motion.div className="h-full bg-gradient-to-r from-teal-400 to-green-500 rounded-full"
                  initial={{ width: 0 }} animate={{ width: showAll ? '100%' : `${navProgress}%` }} transition={{ duration: 0.3 }} />
              </div>
              <button onClick={() => setShowAll(v => !v)}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold uppercase tracking-widest border-2 transition-all duration-200 select-none whitespace-nowrap
                  ${showAll ? 'bg-teal-500 border-teal-400 text-gray-900' : 'bg-gray-800 border-gray-700 text-teal-400 hover:border-teal-600'}`}>
            ...
            {!showAll && (
              <div className="flex justify-center gap-2 mb-8">
                {groupedStats.map((g, i) => (
                  <div key={g.category}
                    className={`h-2 rounded-full transition-all duration-300 ${i === categoryIndex ? 'w-8 bg-teal-400' : i < categoryIndex ? 'w-4 bg-teal-700' : 'w-4 bg-gray-700'}`} />
                ))}
              </div>
            )}
                  ...
                  <motion.div key={categoryIndex} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}
                    ...
                    <div className="flex flex-col gap-3">
                      {currentGroup.stats.map((stat, i) => (
                        <motion.div key={stat.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
                          <StatRow stat={stat} value={getValue(stat.id)} onBump={bump} accent={categoryAccent[currentGroup.category]} />
                        </motion.div>
                      ))}
                    </div>
                    <div className="flex items-center justify-center gap-4 mt-8">
                      ...
                      <Button onClick={handleNext}
                        ...
                        {categoryIndex === totalCategories - 1
                          ? <><Check className="w-5 h-5 mr-2" />Confirm Stats</>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="flex-1 pb-28 w-full">
                <div className="w-full" style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`, gap: '1rem', alignItems: 'start' }}>
                  {groupedStats.map(group => (
                    <CategoryColumn key={group.category} group={group} getValue={getValue} bump={bump} />
                  ))}
                </div>
                <div className="flex justify-center mt-4">
                  <Button onClick={() => onStatsExtracted({ ...values })}
                    ...
                    <Check className="w-5 h-5 mr-2" /> Confirm Stats & Get Rating
                  </Button>
                </div>
              </motion.div>
            )}
